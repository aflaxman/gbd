...
def fit_emp_prior(id, param_type, map_only=False):
    ...
    fit_model.fit_data_model(vars, iter=10001, burn=1000, thin=9, tune_interval=100)

    for a in dismod3.settings.gbd_regions:
        ...
        for s in dismod3.settings.gbd_sexes:
            for y in dismod3.settings.gbd_years:
                emp_priors = covariate_model.predict_for(model.output_template, model.hierarchy,
                                                            'all', 'total', 'all',
                                                            dismod3.utils.clean(a),
                                                            dismod3.utils.clean(s),
                                                            int(y), vars)
                dm.set_mcmc('emp_prior_mean', dismod3.utils.gbd_key_for(param_type, a, y, s), emp_priors.mean(0))

...
\begin{figure}[h]
\begin{center}
\includegraphics[width=\textwidth]{schiz_forest.png}
...
$1,000$, then the binomial model predicts that a third study
conducted in subpopulation C will have prevalence of four per thousand,
with uncertainty interval $[3,5]$.  I have no problem with the point
...
a unitless ratio of cases to population size, having nonzero
probability of values greater than one is unnecessary.  But for
...
Adding a small small, such as one-half is an alternative solution, and
...
investigation. As discussed in Section~\ref{transformed-normal-models}, there are a number of ad hoc approaches that have been
developed to allow data that include rates of zero to be fit in
...
nonzero value. However, it is clear that these approaches will
...
condition that has population prevalence of one in $10,000$.  Since
...
studies by deterministically including four studies that measured
...
likelihood where each rate has value zero with probability $\phi$ and
value given by a beta-binomial distribution (which may also have value
zero) with probability $1-\phi$.
...
\begin{figure}
\includegraphics[width=\textwidth]{zero_forest.png}

...
\[
\dens(\lambda \given \pi, \delta) \propto \lambda^{\delta-1} \exp\left(-\lambda \pi/\delta \right).
\]
The identity
\[
\frac{\Gamma(k+\delta)}{\Gamma(\delta)k!} \left(\frac{\delta}{\pi+\delta}\right)^\delta \left(\frac{\pi}{\pi+\delta}\right)^k
...
Adding a small offset, such as one-half is an alternative solution, and
...
approach.  I used the approach described in Chapter~\ref{numerical-algorithms} to generate $1000$ samples of the model parameters from the join posterior distribution for each model, using random subsets the data on epilepsy
prevalence collected from systematic review\cite{TK} in the model likelihood.  This dataset

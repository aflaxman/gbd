...
A central decision when modeling sparse, noisy epidemological data with Bayesian methods is the choice of 
the data likelihood function.  That is why this section begins with a discussion of the
alternative models for the metaanalysis of rates, ratios, and durations.
...
The key to connecting the data for different epidemiological parameters, such as incidence and prevalence, is the systems dynamics model from
Chapter~\ref{theory-system_dynamics}.  This \emph{model of process} describes how the \emph{models of data} are related to each other.
I will return to the model of process once the model of data is fully developed.  The model of data is a
...
An example will make this clearer, so I turn now to the metaanalysis
...
metaanalysis, this is a ``fixed effect metaanalysis'', because the
rate is modeled as fixed across all populations.\ref{metaanalysis_book}
...
Another way to quantify the mismatch between the binomial rate model and the observed data is through the posterior
...
effects model in traditional metaanalysis\ref{metaanalysis_book} (or in linear regression\ref{stats_book}).  By introducing additional
...
To this end, I compared all models using a hold-out cross validation

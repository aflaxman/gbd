...
\begin{align*}
r_i n_i &\sim \NegativeBinomial(\pi_i n_i, \delta_i)\\
\pi_i &= \int_{a_{0,i}}^{a_{1,i}} \boldpi(a)dw_i(a)\\
\boldpi(a) &\sim PCGP(\{a_1, \ldota, a_A\}; \boldmu, \scC_\rho)\\
\boldmu(a) &= \exp\left\{\alpha_s + \alpha_t
\end{align*}

\begin{align*}
\boldpi(a_1), \ldots, \boldpi(a_A) \given \mu, \rho &\sim \Normal(\mu, \scC_\rho);\\

...
for mesh_width in mesh_spacing:
    for het_i in heterogeneity:
        ...
        dm.params['global_priors']['heterogeneity']['prevalence'] = het_i
        ...
        models[mesh_width, het_i] = dm
        results[mesh_width, het_i] = dict(rate_stoch=dm.vars['rate_stoch'].stats(), dispersion=dm.vars['dispersion'].stats())

### @export 'save'
pl.figure(**book_graphics.half_page_params)

for row, mesh_width in enumerate(mesh_spacing):
    for col, het in enumerate(heterogeneity):
        index = row*3+col

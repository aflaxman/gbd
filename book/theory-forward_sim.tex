...
birthrate $\beta$ and decreasing with mortality rate $\mu$. When $\beta$ and $\mu$
...
first glance.  All of the parameters of in the model are functions of
...
\begin{align*}
\frac{\d }{\d \t} S(a+\tau,t+\tau) &= -(i + m)S + rC,\\
\frac{\d}{\d \t} C(a+\tau,t+\tau) &= iS - (r + m + f)C,\\
\end{align*}
where $S,C,i,r,m,$ and $f$ are shorthand for the following functions of age and time:
\begin{align*}
...
measurement of the incidence rate $i$ in the figure above.
...
of age and time, and the mortality rates $m$ and $f$ are small, the duration
simplifies to $X(a,t) \approx \int_{\tau \geq 0} e^{-r\tau}\d \tau =
...
of estimation, we assume that the disease parameters are not changing
substantially with respect to time. To be precise, this assumption
...
instructive to apply it to the ``forward problem'' to show how rates on incidence,
...
. The incidence rate increases linearly from age 0 to
100, and the remission and excess mortality rates are constant with
...
\caption{Consistent disease parameters for a condition where incidence
  ...
  and excess mortality ($f$) rates are constant. The all-cause
  mortality ($m_{\all}$) has an age-specific rate corresponding to females in
  the sub-Saharan, Southern region in 1990. For a condition with
  prevalence of zero at age $0$, these rates lead to a nearly linear
...
\caption{Consistent disease parameters for a condition where incidence
  ...
  function of age while remission ($r$) is constant. The all-cause
  mortality ($m_{\all}$) has an age-specific rate corresponding to females in
  the Southern sub-Saharan Africa region in 1990. For a condition
  with prevalence of zero at age $0$, these rates drive a prevalence
...
also change the shape dramatically. Figure~\ref{forward-sim-birth-prevalence} shows the results of the same
...
\caption{Consistent disease parameters for a condition with nonzero
  ...
  Africa, Southern region in 1990.

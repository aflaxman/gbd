...
The data available for these parameters varies widely
...
\caption{Consistent disease parameters for a condition where incidence
  ($h_i$) increases linearly as a function of age, while remission ($h_r$)
  and excess mortality ($h_f$) hazards are constant. The all-cause
  mortality ($h_{m_{\all}}$) has an age-specific hazard corresponding to females in
  the sub-Saharan, Southern region in 1990. For a condition with
  prevalence of zero at age $0$, these hazards lead to a nearly linear

...
In my meta-regression model of disease in populations, covariate
...
For this meta-regression framework, I have used fixed and
...
gathered with alternative recall periods, an application that arises
...
many studies ask about the existing of symptoms in the past month,
...
here in two variations.  First, when the past year prevalence is used
as the reference value, and second when the past month prevalence is
used as the reference.  This changes the predicted prevalence, of
course, but it also changes the predicted incidence (for which little
data is available).  Figure~\ref{bipolar-ref-alts} shows how the
...
Conceptually, this deserves a separate treatment, however, because the
use and the results are quite different.

A case where the benefit of using fixed effects to predict out of
sample is clear is when modeling an often fatal condition, like
...
introduce fixed effect coefficient analogous to the previous sections,

...
pl.subplot(1,2,2)
sum = pl.array(scatter['compartmental']) + pl.array(scatter['spline'])
diff = pl.array(scatter['spline']) - pl.array(scatter['compartmental'])
pl.plot(sum, diff, 'ks', label = 'GBD 2010 Study Region')
#pl.plot([-1,1], [-1,1], 'k-')
pl.xlabel('Spline + compartmental \nprevalence estimates (%)')
pl.ylabel('Spline - compartmental \nprevalence estimates (%)')
pl.yticks([-.00005, -.0001, -.00015, -.0002, -.00025], [-.005, -.01, -.015, -.02, -.025])
pl.xticks([.001, .0012, .0014, .0016], [.1, .12, 0.14, 0.16])
pl.axis([.0009,.0017,-.00027,-.00004])
pl.subplots_adjust(top=.99, bottom=.27, wspace=.35)
pl.legend(loc='upper right', fancybox=True, shadow=True, numpoints=1, bbox_to_anchor=(.5,-.2))

pl.savefig('book/graphics/ckd-asp_compare.pdf')
pl.savefig('book/graphics/ckd-asp_compare.png')
